...
import { Box, Stack, TextField, Tooltip, Chip, useMediaQuery } from '@mui/material';
...
export const AthleteInformationTable = React.memo(function AthleteInformationTable({
  ...
  const columns = React.useMemo<GridColDef<User>[]>(
    () => [
      ...
      { field: 'email', headerName: t('athletes.information.table.columns.email'), flex: 1 },
      { field: 'phone', headerName: t('athletes.information.table.columns.phone'), flex: 0.8 },
      { field: 'type', headerName: t('athletes.information.table.columns.type'), flex: 0.6 },
      {
        field: 'is_active',
        ...
        ? [
          ...
          },
          {
            ...
            valueGetter: (params) => {
              ...
              const country = params?.value?.country ?? '';
