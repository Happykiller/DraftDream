// src/components/prospects/ProspectSourceTable.tsx
...
import type { ClientSource } from '@hooks/useClientSources';
...
export interface ProspectSourceTableProps {
  ...
  loading: boolean;
  onCreate: () => void;
  onEdit: (row: ClientSource) => void;
  onDelete: (row: ClientSource) => void;
  onQueryChange: (q: string) => void;
  onPageChange: (page: number) => void; // 1-based
  onLimitChange: (limit: number) => void;
}

export function ProspectSourceTable(props: ProspectSourceTableProps): React.JSX.Element {
  const {
    rows,
    total,
    page,
  ...
  const columns = React.useMemo<GridColDef<ClientSource>[]>(
    () => [
  ...
  return (
    <Box sx={{ width: '100%' }}>
      ...
      >
        <TextField
          placeholder={t('prospects.sources.search_placeholder')}
          value={q}
      ...
      <DataGrid
        ...
        autoHeight
        aria-label="prospect-sources-table"
