// src/components/prospects/ProspectStatusTable.tsx
...
export interface ProspectStatusTableProps {
  rows: ClientStatus[];
  total: number;
  page: number; // 1-based
  limit: number;
  ...
  loading: boolean;
  onCreate: () => void;
  onEdit: (row: ClientStatus) => void;
  onDelete: (row: ClientStatus) => void;
  onQueryChange: (q: string) => void;
  onPageChange: (page: number) => void; // 1-based
  onLimitChange: (limit: number) => void;
}

export function ProspectStatusTable(props: ProspectStatusTableProps): React.JSX.Element {
  const {
    rows,
    total,
    page,
  ...
  const columns = React.useMemo<GridColDef<ClientStatus>[]>(
    () => [
  ...
  return (
    <Box sx={{ width: '100%' }}>
      ...
      >
        <TextField
          placeholder={t('prospects.statuses.search_placeholder')}
          value={q}
          onChange={(event) => onQueryChange(event.target.value)}
          inputProps={{ 'aria-label': 'search-statuses' }}
          size="small"
          sx={{ maxWidth: 360 }}
        />
        <Box sx={{ flex: 1 }} />
        <Button variant="contained" onClick={onCreate}>
          {t('prospects.statuses.create')}
        </Button>
      </Stack>
      ...
      <DataGrid
        ...
        autoHeight
        aria-label="prospect-statuses-table"
