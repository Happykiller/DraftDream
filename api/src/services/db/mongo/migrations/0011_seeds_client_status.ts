// src/services/db/mongo/migrations/0011_seeds_client_status.ts
...
const migration: Migration = {
  ...
  description: 'Seed default client statuses (locale=fr) and link to admin user as createdBy',

  async up(db: Db, log) {
    const col = db.collection('client_statuses');
    await col.createIndexes([
      { key: { slug: 1, locale: 1 }, name: 'client_statuses_slug_locale', unique: true },
      { key: { updatedAt: -1 }, name: 'client_statuses_updatedAt' },
    ]);
    log('client_statuses indexes ensured');
    ...
    const ops = FR_STATUSES.map((label) => {
    ...
    log(`client_statuses upserted: inserted=${res.upsertedCount ?? 0}, modified=${res.modifiedCount ?? 0}`);
  },
};
