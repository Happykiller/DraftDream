...
const migration: Migration = {
  id: '0008_backfill_muscle_name',
  description: 'Ensure all muscles have a populated name field',

  async up(db: Db, log) {
    const col = db.collection('muscles');
    ...
    for await (const doc of cursor) {
      ...
      updates.push({
        updateOne: {
          filter: { _id: doc._id },
          update: { $set: { name: derivedName || doc.slug || 'Muscle' } },
        },
      });
    }

    if (updates.length === 0) {
      log('No muscles missing name field.');
      return;
    }

    const res = await col.bulkWrite(updates, { ordered: false });
    log(
      `muscles name backfill: matched=${res.matchedCount ?? 0}, modified=${res.modifiedCount ?? 0}`
    );
  },
};
