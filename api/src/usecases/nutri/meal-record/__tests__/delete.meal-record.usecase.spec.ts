...
describe('DeleteMealRecordUsecase', () => {
  ...
  beforeEach(() => {
    ...
    usecase = new DeleteMealRecordUsecase(inversifyMock);
  });

  it('should delete a meal record for the creator', async () => {
    asMock(mealRecordRepositoryMock.get).mockResolvedValue(record);
    asMock(mealRecordRepositoryMock.softDelete).mockResolvedValue(true);
    ...
    expect(asMock(mealRecordRepositoryMock.softDelete).mock.calls[0][0]).toBe('record-1');
  });
