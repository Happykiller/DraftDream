...
describe('DeleteProgramRecordUsecase', () => {
    ...
    let usecase: DeleteProgramRecordUsecase;
    ...
    beforeEach(() => {
        ...
        usecase = new DeleteProgramRecordUsecase(inversifyMock);
    });

    it('should soft delete a program record if user is owner', async () => {
        asMock(programRecordRepositoryMock.get).mockResolvedValue(record);
        asMock(programRecordRepositoryMock.softDelete).mockResolvedValue(true);

        const result = await usecase.execute(dto);

        expect(result).toBe(true);
        const softDeleteMock = asMock(programRecordRepositoryMock.softDelete);
        expect(softDeleteMock.mock.calls.length).toBe(1);
        expect(softDeleteMock.mock.calls[0][0]).toBe('record-1');
    });

    it('should soft delete a program record if user is admin', async () => {
        asMock(programRecordRepositoryMock.get).mockResolvedValue(record);
        asMock(programRecordRepositoryMock.softDelete).mockResolvedValue(true);
    ...
    it('should throw unauthorized if user is not owner and not admin', async () => {
        ...
        expect(asMock(programRecordRepositoryMock.softDelete).mock.calls.length).toBe(0);
    });
    ...
    it('should return false if record not found and user is admin', async () => {
        asMock(programRecordRepositoryMock.get).mockResolvedValue(null);

        // Admin trying to delete non-existent record - use case returns false if not found for admin
        const result = await usecase.execute({
            ...dto,
            session: { userId: 'admin-1', role: Role.ADMIN },
        });
