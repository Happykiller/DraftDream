...
/**
 * Confirms deletion of a private session template from the library.
 */
export function ProgramBuilderDeleteSessionDialog({
  open,
  sessionLabel,
  onClose,
  onConfirm,
}: ProgramBuilderDeleteSessionDialogProps): React.JSX.Element {
  ...
  const handleConfirm = React.useCallback(async () => {
    ...
    try {
      ...
      setErrorMessage(t('programs-coatch.builder.delete_session_dialog.errors.delete_failed'));
    } finally {
      setIsSubmitting(false);
    }
  }, [isSubmitting, onClose, onConfirm, t]);

  const dialogTitle = t('programs-coatch.builder.delete_session_dialog.title');
  const dialogSubtitle = t('programs-coatch.builder.delete_session_dialog.subtitle', {
    label: sessionLabel,
  });
